...
> "Our finance team spends three days a month keying supplier invoices. Walk me through what you would build — and tell me exactly where a person has to approve, and why there."
...
By the end of today you can pick a process on stated grounds rather than on which department complained loudest; map it at a resolution that survives contact with the second case; put the approval where the run stops being undoable and defend the *number* of approvals with arithmetic from the customer's own volumes; and say what "last mile" means, where the phrase comes from, and why anybody pays for it.
...
By lunchtime the sheet has stopped being true. She files a burr off the second face that the sheet does not mention, because the supplier's castings changed last spring and nobody reissued the sheet. She rejects roughly one casting in twelve on a call she makes by feel; ask her the rule and she says "you can just tell". "Inspect" turns out to be two operations, because the bore gauge drifts after about forty parts. And every third or fourth part is a special: a rush order that skips the deburr, a rework that comes back already drilled, a batch quarantined because the heat number is wrong.
...
The **last mile** in `GLOSSARY.md` is the gap between a general capability and one customer's mess. Today gives it a concrete meaning. The general capability is a model that can read an invoice; everybody can buy the same one. The last mile is *this region always needs a second look* — a rule in one operator's head, true for your customer and nobody else, without which the system is wrong on a slice of cases and cannot tell you which slice. It is different for every customer, which is why the role is a person and not a document.
...
That placement is the whole point. Day 1's **stopping condition** already had three exits: the model declaring it is done, the cap firing, and an external interrupt. The approval pause is that third exit, and it is the only one that is *planned*. Which gives you the distinction an interviewer will test:

| | A **tripwire** (Day 3) | An **approval pause** (today) |
|---|---|---|
| What triggers it | A check failed | The design says so, every time |
| State of the run | Broken | Fine, and waiting |
| What the person receives | An incident | A decision |
| What happens next | The run is over | The run continues from where it stopped |
...
**So the approval pause is the first thing in this course that makes durable state mandatory rather than a choice.** Day 4 named the **checkpointer** and described it thinly; the mechanics of saving state mid-run and picking it back up are Week 2's, and this is a large part of why Week 2 exists. Saying that out loud in an interview shows you know what your own design costs.
...
- Only steps past the reversibility boundary can require one. A read leaves the customer's systems as it found them; so does a draft that is assembled and not posted. Nothing before the boundary needs permission, because nothing before it has happened to them.
...
<img src="diagrams/reversibility-line.svg" alt="One run of an invoice agent: four steps the agent performs alone, a dashed reversibility boundary, the approval pause straddling it, and two writes past it" width="100%">

*What to notice: the boundary is not drawn between safe steps and risky ones. It is drawn where undoing stops being possible. Two writes sit past it and one pause covers both, because the supplier email is judged on the same evidence as the posting — so the floor for this run is one, not two.*
...
**The ceiling comes from the customer's staffing, and this is the half nobody prepares.** Every pause consumes a person's attention. So if the process runs *R* times per period, each approval takes *t* minutes of genuine review, and the customer will actually staff *M* minutes per period for approving, then the approvals one run may carry are bounded by:

`a ≤ M ÷ (R × t)`

Work it on the invoice process. Say 120 invoices a day. One accounts-payable clerk is giving this two hours a day, so *M* is 120 minutes. Reading a drafted record against the invoice and the purchase order well enough to catch a wrong cost centre takes about ninety seconds, so *t* is 1.5. Then `a ≤ 120 ÷ (120 × 1.5) = 120 ÷ 180 = 0.67`.

**Which is not an approval count.** You cannot pause 0.67 times, and there is no way to round it: 1 exceeds the ceiling and 0 is below the floor. The arithmetic has told you something the number itself cannot. **At this volume, an approval on every run is not staffable, and no amount of tuning the design gets there.** Floor 1, ceiling 0.67, and the two do not meet. That is a finding, it arrives before you have built anything, and that is the only time it is cheap.
...
1. **Approve by exception.** A rule decides which runs a person sees, and the rest proceed without one. *R* in the sum becomes *selected* runs: if 15% of invoices are over a value threshold or involve an unfamiliar payee, 18 runs a day reach a person and `a ≤ 120 ÷ (18 × 1.5) = 4.4`, so the design fits. Say the cost out loud: for the other 85%, the *rule* is the control, not the person. Anthropic's whitepaper supplies the question to ask before accepting that — "if you need to explain exactly why the system made a specific decision to auditors, regulators, or executives, you want predictable, traceable behavior." A selection rule you wrote is traceable; human judgment on a sixth of cases is not a substitute for it.
...
3. **Batch.** One decision covering twenty runs changes *R* from runs to batches. The price is **cycle time**: a batch draining on Fridays is not a pause at all, it is a second backlog with your name on it.
4. **Narrow the scope until the floor drops.** The 20-invoices-a-day version above is this move. A **vertical slice** covering one supplier group end to end, approvals included, is deployable; the same design across all suppliers is not. This is usually the right answer, and the one candidates are most reluctant to give, because it sounds like retreating.
...
**One nesting, named so you do not derive it twice.** Day 3's action budget caps consequential writes per run, and Day 3 derived it. If that budget is eight and *a* is one, then one approval is standing in front of eight writes, so the screen has to show all eight — which raises *t*, which lowers the ceiling. What today adds is only that the two numbers are not independent: moving one moves the other.

**Where the value goes.** The pause is the likeliest place for the project's benefit to leak away: keying that took three days a month now takes four seconds and waits six hours in a queue. The number that survives contact with a customer is **cycle time** — invoice arriving to record posted, including the wait.
...
Nine lines — `PO` on line 3 is the purchase order. Five before the pause, none of which changes anything in the customer's systems. The pause. Then two writes and a closed record. Against Day 5 it is a trace with an approval in it; against Tier 3, the reversibility boundary sits between line 5 and line 8.

**The line worth quoting in an interview:** "Turning a thumbs-up into an auditable control — who, what, when, on what evidence — is most of what risk and compliance teams actually care about." It moves the approval from a safety feature to an audit deliverable, which is the version a customer's risk team will fund.

**Skip if:** you want a method — there is none here, and two honesty items matter more than the summary. The run log is an illustration, not a recording: a scripted animation with an Approve button that advances it, so take it as a well-chosen example of the *shape* rather than evidence about a deployed system. And the site sells a programme, so its claims about what an audit is worth are sales copy. What survives that discount is the specificity of the five-step detail, too particular to have been invented and matching what the podcast in §7 describes at length.
...
**The one idea to take:** read it as a specification of what your interviewer believes the role is, because it is written *for the person hiring you* — most of it is a founder's guide to whether to build an FDE team, how to interview for one and how to scope it, so it tells you unusually directly what will be assessed. The French-restaurant origin and the last-mile framing are in the opening, as is the sentence separating this role from a solutions consultant: "the FDE is still very much an engineer who writes and debugs production code."

The passage most relevant to today is Shilpa Balaji — a former Palantir FDE who went on to lead FDE recruiting there — on weeks spent at a customer's factory: "What you discover onsite is going to be so different from what was sold in the contract". That is Tier 1's resolution gap described by somebody who lived in it.
...
**Skip if:** you want the engineering. There is none. Two cautions. The [essay](https://review.firstround.com/so-you-want-to-hire-a-forward-deployed-engineer/) reports that "Monthly job listings for the role shot up by 800% from January to September of 2025" without saying where the figure comes from, and `FDE_Report`'s own caveats list the 800% claim among the marketing numbers — so cite the essay as the place it is attributable, and do not present it as measured. And it is not a sales pitch for the role: one of its own panellists calls the model "a pretty blunt instrument to try to use for your entire business".

### Anthropic — "Building Effective AI Agents: Architecture Patterns and Implementation Frameworks"
**What it is:** Whitepaper, five pages of dense two-column layout, ~1 hr, free. [PDF](https://resources.anthropic.com/hubfs/Building%20Effective%20AI%20Agents-%20Architecture%20Patterns%20and%20Implementation%20Frameworks.pdf) · checked July 2026.

**The one idea to take:** the human-review pause is named as a **stopping condition** in the loop, not an interface feature bolted on afterwards. That placement is what makes the pause a design object you can reason about, and it is why Tier 2 could line it up against Day 1's other two exits.
...
**Skip if:** you are looking for evidence, and this is worth being blunt about, because the temptation to quote from it is strong. The first two chapters are a run of customer outcomes with no methodology attached: a security-automation platform whose agents are described as "corresponding to 100x time-to-value improvement", resolution rates, percentage productivity gains. These are customers' own numbers as supplied to their vendor. They are not measurements you can defend, and quoting one invites the follow-up you cannot answer — against what baseline, over what period, measured by whom. Take the framing and leave the numbers.
...
One thing you cannot get on paper and should not claim: that you ran it. If a portfolio build is not something you will do, the honest interview version is to present the map and the derivation as analysis rather than deployment, and to name which parts you would expect to be wrong. That is a stronger position than a vague claim to have built something, because the first follow-up resolves it either way.
...
| **The pause went to a group** | Routed to a shared channel or a distribution list. Everybody assumes somebody else looked. Nobody did, and afterwards nobody is accountable. | One named person per pause, with a named deputy and a timeout that escalates rather than proceeding. A pause whose timeout proceeds is a witness point that has been mislabelled. |
| **You automated the step, not the process** | The agent does step 3 flawlessly. A person still re-keys steps 2 and 5 by hand, so the three days a month are still three days a month. | Scope by end-to-end path, not by the step that was easiest to automate. If the manual work either side survives, the saving is zero however well the automated step performs. |

One pattern runs through the table. In the queue, the rejected write, the unrecorded approval, the group routing and the surviving manual work, the model does exactly what it was asked to do and the deployment fails anyway. That is the argument for this day, and for the role: the hard part of putting an agent on a real process is rarely the agent.
...
**Before spending fifty minutes on it:** the title's figure is a claim, not audited data — `FDE_Report`'s own caveats list the $1M number among the marketing claims, sourced to coaching sites and to the interviewee. Chapter `11:26`, "What FDEs Earn" (chapter marker), is the one to skip on those grounds.
...
### "Our finance team spends three days a month keying supplier invoices. What would you build?"
...
**Strong:** "I can give you the shape, but the first thing I'd do is sit with whoever does it, because the version I'd design from this conversation would miss the exceptions, and in a process like this about half the incoming messages carry one. The shape: intake and validation as ordinary code, the agent reads and drafts, one approval, then the write. What I'd be listening for is where the rules live — in a process like this, 'an email arrives' is usually forty senders with no two formats alike and about half of them exceptions somebody resolves from memory, so I'd want the list of unwritten rules for what counts and I'd expect it to take days to get. Then I'd draw the line where undoing stops being possible: reading the purchase order and drafting the record change nothing on your side, posting to the ERP does. That's where the approval goes. And there's already an approval in your process today — a thumbs-up in a direct message, with no record of who approved what. I'm not adding a control, I'm making an existing one auditable."
...
**Why the strong one lands:** it produces a number from the customer's own volumes, and then the number tells it something the candidate did not know at the start. Getting an impossible answer and reading it correctly is a stronger signal than getting a plausible one: it shows the arithmetic is load-bearing rather than decorative.

You should recognise these conversations when a customer opens one, and be able to say which shape their process already is. If they describe it in five steps with no exceptions, they have given you the summary — ask what happens to the cases that do not fit. If they say they already have human oversight, ask whether the work stops or merely gets reported. If they say the process is fully documented, ask who last changed the document.
...
| **Hold point / witness point** | A hold point stops the work until somebody decides. A witness point tells somebody, and the work proceeds. | "We have human oversight" hides which one you built. Only one of them is a control. |
...
| **Approval by exception** | A rule selects which runs a person sees; the rest proceed without one. | The usual answer when per-run approval is not staffable — and it means the rule you wrote is the control on the majority of cases. Say so out loud. |
...
<details>
<summary><b>Q1.</b> What is wrong with designing from the five-box version of a process, if the five boxes are accurate?</summary>

Nothing is wrong with the boxes; they are what the work is supposed to be. What you build against is the boxes plus every exception, and the exceptions are not a small residue — roughly half the incoming invoice messages carry one, and the rule for handling them lives in a person's head rather than in any system. Varick's version: "The documented process is rarely the real process." Build from the summary and the system is correct on the cases the summary describes and silently wrong on the rest.
...
Because the number they experience is cycle time — arrival to posted, including every wait — and the wait is now concentrated in the approval queue. If the pause averages six hours, the four seconds are irrelevant. The second version of the same failure is automating the easiest step rather than the end-to-end path, so a person still re-keys the steps either side and the three days a month survive intact. Quote end-to-end time from the first week, and scope by path rather than by step.
