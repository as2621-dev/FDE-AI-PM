...
_Backoff, jitter, and not making an outage worse by retrying into it._

Write `course/day-15-retry-logic/README.md` — a 45–60 minute teaching document that takes a
...
**Owns:** Exponential backoff, the thundering herd, retry budgets/token bucket, retry storms across layers

**Must NOT cover:** Which errors to retry (D10), idempotency (D13)

**Gap fill (required, from the report's own gap table):** **Jitter** — the report's own 1·2·4·8·16s schedule omits it. Full/equal/decorrelated jitter
...
|---|---|---|---|---|
| **AWS, "Exponential Backoff And Jitter"** (Marc Brooker, AWS Architecture Blog) | Essay | Full / equal / decorrelated jitter formulas; the classic thundering-herd fix | Free | 45 min |
| Google SRE — retry budgets / token bucket | Book | Avoiding retry storms across layers | Free | 45 min |

**Exercise:** Exponential backoff on every external call — 1s · 2s · 4s · 8s, cap at 16s — and add jitter (the plan omits jitter; add it, per AWS).
