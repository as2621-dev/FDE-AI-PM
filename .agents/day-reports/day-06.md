...
- **Gate stats (final, after review fixes):** **9,933 prose words / 8 Q&A / 2 videos / 7 links / 2 diagrams / 16 vocab terms.** PASS **without** `--offline`, first attempt, no dead links. `course/build.py` succeeds; `index.html` carries both SVGs inlined with captions, 12 `<figcaption>`s across 6 days, zero `<p><figure>` nesting, zero `class=` inside any inlined SVG, zero external hosts referenced from any SVG.

  Two warnings, both expected:
  1. **9,933 words, at 66 estimated minutes against a 45–60 minute promise.** This is the headline finding of the slice — see the length section below. Justification for the band: the overflow is four owned subtopics plus a 16-term vocabulary paid from zero, not restatement.
  2. **Two distinct figures "cited with no link".** Both are structural false positives of a kind worth recording, because they are the opposite of what the check hunts: `$1M` sits in a paragraph whose entire purpose is to say the number is a marketing claim, and `1%` is a hypothetical inside a §10 question stem. A third hit, `800%`, **was the check doing its job and it found a real defect underneath** — the paragraph asserted that the First Round essay gives no source for the figure, and the essay in fact hyperlinks it to the Financial Times. So this check's one non-false-positive on this day led directly to a false absence claim being caught. **Worth adding to Day 5's assessment: it is not just noise.**
...
**The strongest single thread for "walk me through your agent", if Day 7 wants one.** The **stopping condition**. Day 1 *taught* two exits and its vocabulary named a third (`external interrupt`) without teaching it — a gap the review panel found in my draft's characterisation and which is worth Day 7 knowing; Day 3 added the tripwire as the *unplanned* exit; Day 5 showed that the step cap's exit is indistinguishable from success in the OpenTelemetry conventions and has to be a custom field; Day 6 made the approval pause the *planned* third exit — the one Day 1 named and never taught — and drew the tripwire-vs-pause distinction as a table. That spine touches four of the six days, it is drillable, and it is the place where a candidate who has actually read the week sounds different from one who has read a blog post.

---

## The length finding, which is the process story of this slice

**The first complete draft was 11,087 prose words against a 10,000 hard ceiling.** Getting to 9,715 pre-review took eight passes, and the numbers confirm `length-reconciliation-strands-terms.md` precisely. (The day then went to 10,025 under review fixes and finished at **9,933** — see the panel section.)
...
2. **A 285-word reserve is not enough, and Day 5 was right to say ~900.** I could not reach 900 without cutting teaching, so I pre-committed to a deletion list instead of a buffer: §10's Q7, §8's second "Why the strong one lands", and Tier 3's cycle-time paragraph. **The panel's fixes then took the day to 10,025 — a hard gate failure — and the pre-committed list was not enough on its own.** Recovery took those deletions plus seven further compressions and one deleted §6 row. So: the plan helped and it did not substitute for the reserve. **Day 5's ~900 figure is now confirmed by a second day, and I would treat anything under 500 as a failure to plan rather than a tight landing.**
...
**3 reviewers, ~40 findings, ~32 applied, 6 deferred or refused** with reasons in `docs/residual-review-findings/f10434a.md`. The most productive panel of the six days so far, and the top findings were all things the reader would have been caught on. Ranked by damage:

1. **The day's headline sentence was false by the day's own arithmetic.** I wrote that at 120 invoices a day "an approval on every run is not staffable, and **no amount of tuning the design gets there**" — then, four paragraphs later, offered "**reduce *t* by designing the screen**" as move 2. At `t = 1.0` the ceiling is `120 ÷ 120 = 1.0`, which *meets* the floor of 1. Two reviewers found it independently; §10 Q3 had the absolute claim and the tuning move in the same paragraph. Now quantified instead of asserted: the ceiling reaches 1 only if *t* falls to a minute, and whether sixty seconds is genuine review of three documents is the question rather than the arithmetic. **This was the single most valuable finding**, because the impossible-value paragraph is the one Rule B's newest clause exists to produce, and mine overstated its own result.
2. **`t` was the only uncited input in the sum, and it alone produced the finding.** Reviewer 2's four-word question — *"Where's ninety seconds from?"* — is exactly the Day 1 failure the style guide records: an uncited number sitting where the reader most needs a defensible one. *t* is now declared a judgment input with a direction to move it, the way Day 5 handled `k`.
3. **`t` was two incompatible quantities.** The sum needs hands-on minutes; Tier 3's "replace the guess" told him to measure *elapsed* time from pause-open to decision, which includes lunch and which the day elsewhere gives as six hours. Substituting that returns `0.0028`. The day now separates them explicitly: *t* is capacity, elapsed time is cycle time, and putting one in the other's sum collapses it.
4. **The premise did not survive its own arithmetic.** The header said the team spends *three days a month* keying. At 120 invoices a day that is ~34 seconds per invoice by hand against 90 seconds to review the agent's draft — so the design cost the clerk 2.6× the status quo, and the business case is deferred to Week 4, so he had nothing to say. Premise corrected to **about ten days a month**, which is what 120 a day at two minutes each actually costs, and Tier 3 now states the comparison out loud: ninety seconds of review is *less* work per invoice than two minutes of keying.
5. **The second write past the boundary was my invention, presented as sourced.** The Varick run log has one write (`posting to the ERP`); the supplier-notification box in `reversibility-line.svg` appears nowhere on the microsite, and §4 read "two writes" out of a nine-line log that has one. Worse, Reviewer 2 showed the invented write also **broke my own merge rule**: §6 says the ERP can reject the write, so what the email says depends on information existing only after the posting, which by my own rule makes the floor 2 rather than 1. Fixed by deleting the invented box — which corrected three findings and the write-count inconsistency at once, and removed words rather than adding them.
6. **Three of the four cells in my tripwire column misstated Day 3.** Day 3 defines a tripwire as "halt the run **and hand it to a person**" and its mitigation is "Route blocks to a human queue, **not a dead end**". I had the person receiving "An incident", the run being "Broken", and "The run is over" — Day 3's *failure* mode presented as its definition. The day stakes itself on that table ("conflating these is the commonest way this answer goes wrong"), so getting the neighbour's column wrong was the worst possible place for it.
7. **The whitepaper is 30 pages, not five — and `file` is why.** `file` reports "PDF document, version 1.7, 5 pages"; `pdfinfo` reports 30, and the printed page numbers run to 29. `file` counts something other than pages on a linearized InDesign PDF. **Never take a page count from `file`.**
8. **My absence claim about the First Round essay was false**, and absence claims are the class §5 rule 7 names as breaking most often. I wrote that the essay reports the 800% figure "without saying where the figure comes from"; the published HTML hyperlinks the phrase to the Financial Times. The remedial advice I built on it — "cite the essay as the place it is attributable" — was therefore wrong advice to carry into a room. Now attributed to the FT via the essay, with the honest note that the FT does not publish the method.

Also applied: `blast radius` and `cycle time` were both **used in bold before §9 defined them** (and `blast radius` was not pre-existing in `GLOSSARY.md` — this day's commit adds it), so both now carry an inline gloss at first use; §9's headword `Approval by exception` did not match the body's `Approve by exception` in any of four places; §9's hold/witness row said "Only one of them is a control" while the body says "Both are legitimate", so the row now matches the body; `a` was never bound to its symbol before the formula used it; "human judgment on a sixth of cases" corrected to 15% (18 ÷ 120); the SVG `<desc>` said "five steps" and listed four, which is the only text a screen-reader user gets; move 1 now re-derives *t* on the selected cases, since selecting the high-value and unfamiliar-payee runs selects the *hard* ones by construction and at eight minutes each the same sum returns 0.83 and fails again; the claim that "nothing before the boundary needs permission" now carries Day 3's caveat that reads are reversible and **not** free, because exfiltration needs no write; the three-way rule triage now defers the *which* to Week 4, since Day 23 owns that framework; two dependencies beyond durable state named (recognising a duplicate invoice, and pulling forty fields out of a screenshot), because my claim that durable state was *the* missing one was too strong; the approval span noted as payload and therefore needing exemption from Day 5's sampling rate; a §6 note that this workflow already has two legs of Day 3's lethal trifecta so any outbound step completes it; §10 Q1 replaced, because it was answerable from its own stem and nothing in the day tested hold point versus witness point; §8's closing reframed from four questions to three recognitions, since supplying questions after deferring the questioning technique was a boundary crossing; and `reading_minutes` corrected from 64 to match the gate.

**One self-inflicted problem, and it is the one Day 5 predicted.** Applying the findings took the day from 9,715 to **10,025 — a hard gate failure.** I had shipped with 285 words of headroom against Day 5's recommendation of ~900, and mitigated with a pre-committed deletion list instead of a buffer. The list was not enough on its own: recovering took the planned deletions **plus** seven further compressions, and one of the string edits introduced a doubled quote mark (`auditable.""`) that only a re-read caught. **Re-run the gate after applying findings, and treat a sub-500-word reserve as a failure to plan.** Final state: 9,933 words, 67 words of headroom, PASS online.

**Every check re-run after the fixes.** All 49 quoted strings re-verified against the corpus (one reported miss is a regex artefact matching the text *between* two adjacent quoted fragments); zero banned phrases; all 15 numeric count-promises re-confirmed against what follows them; every §9 headword confirmed to appear in the body; `GLOSSARY.md` re-synced from the edited §9 (127 A–Z rows, 16 from Day 6, zero duplicates, no pre-existing row altered); `build.py` re-run and `index.html` re-verified (12 `<figcaption>`s, 12 inlined SVGs, zero `class=`, zero external hosts, zero `<p><figure>` nesting, and no stale supplier-email text surviving in the page).
